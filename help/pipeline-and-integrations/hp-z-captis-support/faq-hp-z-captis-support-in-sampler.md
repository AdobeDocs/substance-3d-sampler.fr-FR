...
La solution couvre des cas d&#39;utilisation intersectoriels (Automobile, Vêtement, Conception de produits, Média et divertissement, Architecture...). Le mode Studio permet la capture sur ordinateur (reproductible, efficace et simple), tandis que le mode Explorateur permet la capture sur mobile (flexible, mobile, s&#39;adaptant à chaque situation).

+++

+++Quels types de matériaux peuvent être numérisés et capturés avec Captis ?
Tous les types de matériaux peuvent être scannés et capturés, sauf avec plusieurs couches de revêtement transparentes (les peintures de voiture sont exclues du champ d&#39;application de Captis). Certains matériaux spécifiques peuvent nécessiter un traitement supplémentaire dans Sampler pour optimiser les résultats. Veuillez noter que les algorithmes de traitement seront continuellement optimisés au fil du temps.

+++

+++Quelles sont les restrictions sur la taille ou la forme des échantillons matériaux - les échantillons doivent-ils être plats ?
Captis peut numériser une grande variété d’échantillons de matériau, de taille ou de forme. Il est livré avec des aimants pour aplatir les échantillons sur le plateau d&#39;échantillonnage. Il existe plusieurs modes pour capturer un échantillon de matériau avec Captis :
...
* Mode Explorateur : vous pouvez utiliser la bague explorateur sur le terrain, sur le plateau ou dans des environnements uniques et activer la capture flexible pour les échantillons de plus de 30 cm x 30 cm. Limitation actuelle : veuillez noter que le mode Explorateur est encore une version antérieure et n’est pas encore optimisé (à partir de la version du 29 juillet 2024).
...
* La répétition est effectuée dans la pile de calques Sampler à l’aide des filtres de répétition actuels.

+++

+++Quels canaux PBR sont disponibles ?
À compter de la version du 7 août 2025, les cinq mappages exportés sont Base color, Rugosité, Normal, Height et Opacité. Le pipeline de traitement actuel ne gère pas encore le mappage de métallisation.

+++

+++La répétition est-elle effectuée automatiquement ?
La répétition est effectuée dans la pile de calques Sampler à l’aide des filtres de répétition actuels.

Le filtre répétition automatique peut être utilisé pour placer automatiquement en mosaïque des matériaux présentant une structure répétitive définie ou de petits motifs, avec un minimum de 3 motifs dans chaque direction. Pour en savoir plus sur ce filtre, consultez la [section dédiée de la documentation](../../filters/tools/auto-tiling.md).
...
Par exemple, vous pouvez ajouter 192.168.55.1 captis-device, puis dans <b>Paramètres de Sampler > Stockage et cache > Capture de Matériau > Adresse Captis</b>, remplacer l’adresse IP par captis-device
...
Assurez-vous que la protection de la caméra a été supprimée.
...
Si vous avez demandé à récupérer les images de matériau et de photométrie, il est normal que la copie prenne plus de temps.
...
+++J&#39;ai déplacé le cône de sa base vers l&#39;anneau explorateur et je ne peux plus le scanner.
Il est recommandé d&#39;éteindre le HP Z Captis avant de le débrancher de son socle ou de l&#39;anneau explorateur.

+++

+++Exporter mon matériau dans SBSAR est lent.
...
+++Je veux modifier le chemin d’enregistrement des matériaux capturés et des images de photométrie.
