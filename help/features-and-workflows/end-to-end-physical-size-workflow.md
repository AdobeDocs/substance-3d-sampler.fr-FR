...
breadcrumb-title: ''
description: Découvrez comment utiliser le workflow de taille physique de bout en bout dans Substance 3D Sampler pour créer des matériaux physiquement précis à l’échelle du monde réel.
...
1. Sélectionnez le modèle de création de matériau.
...
1. Ajoutez un recadrage et ajustez-le à l’échantillon. La taille physique affichée dans l’angle inférieur droit du Viewport 2D est mise à jour.

   Affichez les proportions physiques dans le viewport 2D pour voir avec précision les cartes sur lesquelles vous travaillez.\
   Vous pouvez définir la Vue 2D pour l’adapter à la taille physique afin que la valeur PPP de votre rapport d’écran corresponde à votre échelle de matériau. En d&#39;autres termes, vous pouvez placer votre échantillon réel à côté de votre écran pour vérifier les dimensions.

   ![](../assets/cq5dam.web.1280.png)
1. Ajoutez une option Égaliser pour supprimer les dégradés.
1. Ajout de Répétition pour corriger la répétition
1. Si nécessaire, le transforme de déformation est utile pour réaligner uniquement certaines parties de la carte.
