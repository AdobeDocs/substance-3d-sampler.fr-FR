---
helpx_url: "https://helpx.adobe.com/fr/substance-3d-sampler/release-notes/old-versions/version-2020-3-2-3.html"
breadcrumb-title: ''
description: Consultez les notes de mise à jour de Substance 3D Sampler version 2020.3 (2.3) pour en savoir plus sur la création d’atlas, les améliorations des vignettes et la localisation.
helpx_creative_field: ""
helpx_description: Sampler > Release Notes > Old Versions > Version 2020.3 (2.3)
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Version 2020.3 (2.3)
user-guide-description: ''
user-guide-title: ''
source-git-commit: 55277f7a92e97bf530dd2a2edf4e16c88bb57793
workflow-type: tm+mt
source-wordcount: '405'
ht-degree: 0%

---


# Version 2020.3 (2.3)

**Substance Alchemist 2020.3 « Vermicelli »** introduit de nouveaux contenus et de nouveaux workflows comme la création d’atlas à partir d’une seule image. Nous avons également amélioré la qualité des vignettes de nos matériaux et avons localisé l’interface en chinois.

Date de publication : *26 octobre 2020*

## Principales fonctionnalités

### Image en Matériau (optimisé par l’IA)

* Nouveaux paramètres
  * Affinez votre géométrie (couches normale et height) en ajustant les différents niveaux de détails (micro, moyen et grand)
  * Réglez l&#39;intensité du ravissement pour conserver certaines informations dans votre base color
  * Des paramètres de variation et de lissage sont disponibles pour générer une couche de rugosité plus précise
* Prise en charge de la série Nvidia RTX 3000

![](../../assets/materia.jpg)

### Workflow de création d’atlas

Avec l’ajout de deux nouveaux filtres, la création de l’atlas à partir d’une seule image est simplifiée dans la Substance Alchemist.

Générateur d’atlas : le filtre génère automatiquement, à partir de la base color, une couche d’opacité. La diffusion des couleurs est également créée sur la base color. Il est recommandé d’utiliser une image avec un arrière-plan uniforme.

Atlas splitter : Le filtre vous permet d’isoler un élément spécifique de votre atlas et de le redimensionner.

![](../../assets/atlas.jpg)

### Vignettes

Les vignettes sont maintenant générées à l&#39;aide du nœud Rendu PBR de Substance Designer avec le nouveau map d&#39;environnement Studio\_06.

Si la miniature est incorporée dans le fichier d&#39;archive de Substance de données (\*.sbsar), Substance Alchemist lit désormais la miniature et ne la génère pas à nouveau.

![](../../assets/screenshot-new-thumbnails.jpg)

Un nouveau paramètre dans les préférences permet de modifier la qualité des vignettes pour gagner du temps. Par défaut, la qualité est définie sur Haute.

### Localisation en chinois

La Substance Alchemist est maintenant localisée en chinois. Vous pouvez modifier la langue de l’interface dans les Préférences.

![](../../assets/screenshot-chinese.jpg)

### Contenu nouveau et mis à jour

* **Filtres techniques**

Déformation : déformez votre matériau pour casser des motifs ou rendre votre matériau plus vivant.

Relief de surface : ajoutez du relief et de la variation sur votre matériau (remplacez le filtre de modulation d’height)

Inverser : inversez les canaux de votre choix

* **Altération de filtres**

Scratches : ajoutez des rayures sur votre matériau. Utile sur les bois et les métaux par exemple.

Empreintes digitales : ajoutez des empreintes digitales sur des métaux ou des matériaux brillants.

Gommes jetées : Dispersion de vieilles gommes sur un sol

* **Filtres colorimétriques**

Coloriser : Tint d’une partie ou de tout votre matériau

Variation de couleur : permet désormais de sélectionner la façon de segmenter votre matériau. En plus de la base color, vous pouvez utiliser l’height, l’ambient occlusion, le métallique et la rugosité.

Remplacer la couleur : sélectionnez une couleur sur votre Vue 2D et sélectionnez-en une nouvelle.
