...
Fenêtre d’exportation d’une exportation de matériau.
...
* <b>Format</b> : sélectionnez un format de fichier pour votre ressource exportée.
  * <b>SBSAR</b> : exportez votre matériau pour l’utiliser dans toute application prenant en charge les matériaux de Substance.
  * <b>SBS</b> : exportez votre matériau afin de l&#39;ouvrir dans Substance 3D Designer.
  * <b>EXR, JPEG, PNG, TARGA, TIFF</b> : exportez votre matériau sous la forme d’une collection de fichiers image.

>[!NOTE]
>
> Le nombre de bits par pixel est forcé à 16 bits pour les canaux Normal et Height. Les autres canaux sont exportés en 8/16 bits en fonction de vos matériaux et des filtres utilisés par votre ressource. Selon le format de fichier, le nombre de bits par pixel peut être modifié, car certains formats de fichiers ne prennent pas en charge le nombre de bits par pixel élevé.
...
* <b>Paramètre prédéfini </b>(EXR, JPEG, PNG, TARGA, TIFF) : sélectionnez un paramètre prédéfini pour configurer automatiquement l’exportation de fichiers pour une application ou un pipeline donné.
  * L&#39;option <b>Par défaut (workflow du projet)</b> affiche une liste de tous les canaux disponibles de votre ou vos matériaux sans qu&#39;aucun paramètre prédéfini ne soit appliqué.
...
* <b>Type de Matériau </b>(SBSAR, SBS) : indiquez si le matériau exporté doit se comporter comme un matériau, une décalcomanie ou un atlas standard. Ce paramètre peut changer la façon dont il est traité par d&#39;autres applications qui prennent en charge les fichiers SBSAR et SBS.
...
* **Modèle de matériau** (tous les formats dans le paramètre prédéfini par défaut) : sélectionnez une norme de shader pour vos textures exportées.
  * La modification du Modèle de matériau aura un impact sur les noms de fichiers des fichiers exportés. Par exemple, OpenPBR utilise « Metalness » au lieu d&#39;ASM qui utilise « Métallique ».
...
>[!NOTE]
>
> La <b>Taille physique</b> est définie lors de la création du matériau et ne peut pas être modifiée lors de l&#39;exportation.

### Canaux

![](../../../assets/SAPR_Channelspreview.png)

Dans la partie droite du panneau <b>Paramètres de Matériau</b>, une liste des canaux pouvant être exportés et leur résolution est visible (canaux par défaut et canaux personnalisés).
