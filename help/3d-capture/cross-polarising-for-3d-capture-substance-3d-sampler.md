...
Lorsque la lumière atteint une surface, elle se reflète généralement de manière diffuse, rebondissant uniformément, donnant à la surface son aspect coloré. Mais selon la rugosité de la surface, une partie de la lumière peut être réfléchie directement vers votre œil ou votre caméra. Ce <b>reflet de specular</b> change en fonction de votre angle de vue.
...
Lorsque vous êtes correctement configuré, vous devriez être en mesure de capturer un objet comme s’il était complètement mat, sans reflets ni éclairage. Tout comme le fait de voir votre maillage avec la seule texture de couleur de base appliquée, elle vous permet de capturer des objets réfléchissants difficiles.
