...
breadcrumb-title: ''
description: Découvrez comment traiter des captures 3D avancées dans Substance 3D Sampler pour optimiser la géométrie, les textures et la qualité du matériau.
...
Sélectionnez toutes vos photos dans l’explorateur et faites-les glisser sur la fenêtre capture 3D. Après un certain temps de chargement, vos photos sont présentées dans une liste et une galerie, avec les propriétés de la sélection à droite.

La liste des groupes de photos sur la gauche est basée sur la caméra et l’objectif utilisés pour les photos. Si vous mélangez des photos à partir de plusieurs appareils, tels qu&#39;un téléphone portable, une caméra reflex numérique ou un drone, vous obtiendrez <b>des groupes distincts</b> ici.
...
Il ne reste qu’un seul paramètre avant la création de notre maillage. Sous Détails de la géométrie (geometry details), vous pouvez sélectionner le niveau de détail de géométrie initial.
...
* Les <b>brouillons</b> sont des <b>maillages décimés</b>. Choisissez des options plus basses pour obtenir un résultat de test plus rapidement, des options plus élevées pour obtenir plus de détails au détriment d&#39;un traitement plus lent.
...
Une fois votre maillage terminé, la fenêtre finale nous permet de prévisualiser et de post-traiter notre maillage avant que nous l’ajoutions à notre projet Sampler. Ce mode comporte quelques boutons en bas pour voir votre maillage avec une <b>texture</b>, un <b>solide ombré</b>, une <b>structure filaire</b> et un <b>matériau de vérification de l&#39;UV</b>. Les paramètres de post-traitement sur le côté vous permettent de générer une nouvelle version de votre maillage. Cela signifie un maillage réassemblé, avec de nouveaux UV automatiques et une texture bakée du maillage original. Les commandes principales vous permettent de définir un nombre de faces cible et d’activer ou de désactiver les bakings Normal, height et AO. Il existe de nombreux paramètres avancés à modifier, mais les valeurs par défaut fonctionnent généralement correctement.

Vous pouvez également effectuer cette étape de traitement du maillage par la suite, une fois le maillage ajouté à Sampler. Une fois que vous l’avez ajouté à Sampler, vous pouvez lui donner un nom ; il apparaît désormais dans votre liste de projets.

Vous pouvez modifier le maillage et les textures, mais vous pouvez déjà les exporter à l&#39;aide de l&#39;<b>outil de partage</b> Boîte de dialogue > <b>Exporter sous</b>. Les <b>paramètres généraux</b> vous permettent de choisir le nom et le chemin, les <b>paramètres de Maillage</b> vous permettent de choisir le format Maillage 3D et les <b>paramètres de Matériau</b> vous permettent de configurer le matériau du maillage. Vous pouvez désactiver l’option maillage ou matériau pour exporter un seul fichier individuellement. Une fois exporté, votre maillage peut être utilisé dans d’autres applications 3D.
